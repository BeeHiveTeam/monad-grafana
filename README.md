...
В стеке **нет Alertmanager** — и это осознанно. Правила из `prometheus/alerts.yml`
(17 штук) забирает `monad-tg-bot`: он опрашивает `/api/v1/rules` и `/api/v1/alerts`,
пересылает в Telegram только переходы состояния и переиспользует уже готовые у него
доставку, повторы, очередь недоставленного и список разрешённых чатов.

```
Prometheus (17 правил) → /api/v1/alerts → monad-tg-bot → Telegram
```

Дублей нет: то, что бот проверяет напрямую (сервисы, локальный RPC, застой блока, диск),
внесено в его `PROM_SKIP_ALERTS`; из Prometheus берётся то, чего бот сам не видит —
peer-сессии, otelcol, upstream-валидаторы, raptorcast, заполнение TrieDB.

**Без бота правила никуда не уходят.** Они будут гореть внутри Prometheus, и стек будет
выглядеть настроенным, не будучи им. `install.sh` (`verify`) и `scripts/healthcheck.sh`
проверяют это явно и предупреждают — в том числе если бот запущен, но его `bot.py`
старой версии, без моста.

Альтернатива, если Telegram не нужен: поднять свой Alertmanager и добавить секцию
`alerting:` в `prometheus/prometheus.yml` — проверки увидят зарегистрированный
Alertmanager и перестанут ругаться.
